        ...
        {sales.map(s => {
          ...
          return (
            <div key={s.id} style={{
              ...
              {/* Azioni espanse */}
              {isOpen && (
                <div style={{ borderTop: `1px solid ${G.border}`, padding: "12px 14px", display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <button onClick={() => setConfirmDelete(s.id)} style={{
                    padding: "11px 14px", borderRadius: 12, border: `1px solid rgba(255,77,109,.2)`,
                    background: "transparent", color: G.danger, cursor: "pointer", fontSize: 14,
                  }}>🗑</button>
                </div>
              )}
            </div>
